...
class Estudiante extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('cedula, nombres, apellidos, fecha, matricula, user, passwd', 'required'),
			array('cedula', 'length', 'max'=>10),
			array('nombres, apellidos, user, passwd', 'length', 'max'=>100),
			array('matricula', 'length', 'max'=>20),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, cedula, nombres, apellidos, fecha, matricula, user, passwd', 'safe', 'on'=>'search'),
		);
	}
